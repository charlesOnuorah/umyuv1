...
let ChatRoomPage = class ChatRoomPage {
    constructor(router, nativeStorage) {
        ...
        this.nativeStorage.getItem('chatinfo').then(chatInfo => {
            this.roomkey = chatInfo.Key;
            this.username = 'charles onuorah'; //chatInfo.UserName;
            ...
            // });
            this.data.message = '';
            Firebase__WEBPACK_IMPORTED_MODULE_3__["firestore"]().collection('chatrooms/' + this.roomkey + '/chats').onSnapshot(docSnapshot => {
                let chat = [];
                ...
                docSnapshot.forEach(doc => {
                    if (doc.data().message.trim() !== '') {
                        chat.push(Object.assign({ key: doc.id }, doc.data()));
                    }
                });
                console.log('chat before', chat);
                chat.sort((chat1, chat2) => {
    ...
    exitChat() {
        let exitData = Firebase__WEBPACK_IMPORTED_MODULE_3__["database"]().ref('chatrooms/' + this.roomkey + '/chats').push();
        exitData.set({
            type: 'exit',
            user: this.data.username,
